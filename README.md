...
`MapScreen` shows the whole route zoomed out by default (stylized, not a real tile provider — kept the prototype self-contained and fast), a pulsing "you are here" marker that tracks playback progress along the route, numbered stop pins with always-visible name labels, and a distinct dashed line to a labeled "quick exit · main street" — a direct answer to "so they can get out easily," which is a real solo-travel anxiety a generic maps app doesn't address. A Map/List toggle and filter chips (All / AR waypoints / Visited, or Tours / Eat & Shop / Favourites on the city overview) let a traveller narrow what's shown without losing spatial context — non-matching pins dim rather than disappear. Tapping a pin surfaces a card to jump the audio there or open AR, so orientation and control stay in the same place.
